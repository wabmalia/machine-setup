...
| `home/`           | [chezmoi](https://www.chezmoi.io) source state — dotfiles, including generic Claude Code config. |
...
`home/private_dot_claude/` (chezmoi target: `~/.claude/`) tracks the
generic, portable bits of Claude Code config:

- `settings.json.tmpl` — permissions, env, `statusLine`, the iTerm2 status
  hooks, and generic/personal plugins & marketplaces (`claude-mem`, the
  official LSP plugins, and my own `sk1llz` skills repo). Templated so
  absolute paths use `{{ .chezmoi.homeDir }}` instead of a hardcoded
  username.
- `statusline-command.sh` — the custom status line script.

Deliberately **not** tracked here: `CLAUDE.md` and `RTK.md` (both fully
regenerated by `rtk init -g`, see below), `hooks/rtk-rewrite.sh` (same —
rtk owns it), and anything under `projects/`, `sessions/`, `cache/`, or
`history.jsonl` (per-machine runtime state, not config).

`bootstrap.sh` applies these in a specific order so nothing clobbers
anything else: chezmoi writes the generic `settings.json` first, then
`claude-settings.work.json` (if present — see below) is deep-merged on
top via `jq`, then `rtk init -g --auto-patch` runs last to patch in its
own hook entry. Re-running `bootstrap.sh` redoes all three steps
idempotently.
...
| `claude-settings.work.json`                     | work-only Claude Code plugins/marketplaces |
...
only if that file exists — same for the `claude-settings.work.json` merge.

**If you ever change jobs**: delete the six files above, then:
...
- optionally `brew uninstall --cask tuple` (or whatever else was only in
  `Brewfile.work`)
- remove the work entries from `~/.claude/settings.json`'s `enabledPlugins`/
  `extraKnownMarketplaces` by hand (the merge is additive — there's no
  automatic "unmerge")
